...
description: "Learn how to set up and manage a cluster in deployit with docker swarm."
sidebar:
  order: 1
---
...
A cluster is a group of nodes that work together to provide a single, unified service. In deployit, clusters manage a group of servers collaborating to deliver this unified service.
...
- A server with deployit installed.
...
4. Follow the two steps provided:
   ...
   - Install the deployit agent. These steps are the same for both node types.
...
2. Fork the repository [deployit/swarm-test](https://github.com/deployit/swarm-test).
...
To add manager nodes to the cluster, follow the same steps as adding a worker node. However, if the manager node (where deployit is running) fails, all other nodes will become inaccessible. This is because we currently have a single entry point for the cluster. To ensure high availability, you should have multiple manager nodes.
