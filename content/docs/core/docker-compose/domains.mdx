...
1. Add the service to the `deployit-network`.
...
First, we'll add the deployit-network to our services:
...
2. `traefik.http.routers.<UNIQUE-RULE>.rule=Host('your-domain.deployit.com')`
This label tells Traefik that the domain to be used is `your-domain.deployit.com`
...
services:
  frontend:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.frontend-app.rule=Host(`frontend.deployit.com`)
  ...
  backend:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.backend-app.rule=Host(`backend.deployit.com`)
...
2. `traefik.http.routers.<UNIQUE-RULE>.rule=Host('your-domain.deployit.com')` Specifies the domain for the service
