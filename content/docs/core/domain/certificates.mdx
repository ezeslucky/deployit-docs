---
title: "Certificates"
description: "Learn how to set up certificates for the deployit panel"
---

import { Callout } from 'fumadocs-ui/components/callout';

When using a domain for deployit, we offer 3 options for certificates:
...
<Callout title="Cloudflare">When you register a domain on Cloudflare, the SSL certificate is automatically assigned by Cloudflare, so you need to select `None` in deployit and disable HTTPS.</Callout>
...
1. In deployit, select `Let's Encrypt` and enable `HTTPS`.
...
2. In deployit, select `Let's Encrypt` and enable `HTTPS`.
