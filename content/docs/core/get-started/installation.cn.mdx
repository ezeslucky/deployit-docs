---
title: "安装"
description: "借助这本易于上手的安装指南，几分钟内即可在您的服务器上启动并运行deployit。"
---


## 设置您的环境

按照以下步骤在本地设置 deployit 并将其部署到您的服务器， 管理 Docker 容器和应用程序：
...
为了确保 deployit 的流畅体验，您的服务器应该至少有 2 GB RAM 和 30 GB 磁盘空间。该规范有助于处理 Docker 在构建期间消耗的资源并防止系统假死。
...
deployit 利用 Docker，因此在您的服务器上安装 Docker 至关重要。如果尚未安装 Docker，请使用以下命令自动安装：
...
运行安装脚本后，deployit 及其依赖项将在您的服务器上设置。以下是如何完成设置并开始使用 deployit：

### 访问 deployit

打开您的网络浏览器并导航至 `http：//your-ip-from-your-vps：3000`。 您将被引导到初始设置页面，在其中您可以配置 deployit 的管理帐号。

### 初始配置

1. **创建管理员帐号:** 填写必要的详细信息以设置您的管理员帐户。 该帐户将成为 deployit 的管理员帐户。
