---
title: '手动安装'
description: '学习如何在服务器上手动安装 deployit。'
---

如果您希望自定义 deployit 在服务器上的安装，可以修改以下几个方面：

1. **为 deployit 使用特定端口** - 这对于避免与其他服务的冲突非常理想。
2. **选择特定版本的 deployit** - 可以选择如 canary、latest 或特定 Docker 标签（例如 v0.0.1）版本。
3. **指定 Traefik 的 HTTP 和 HTTPS 端口** - 当与其他服务器（如 Nginx）一起运行时非常有用。

## 安装脚本

下面是一个在 Linux 服务器上安装 deployit 的 Bash 脚本。确保在非容器的 Linux 环境中以 root 身份运行，并确保端口 80 和 443 是空闲的。
...
这个脚本包括对常见陷阱的检查，如果未安装 Docker 则安装 Docker，初始化 Docker Swarm，创建网络，然后拉取并部署 deployit。脚本运行后，它提供成功消息和访问 deployit 的说明。
...
如果您的服务器上已经运行了 Docker Swarm，并且您想使用 deployit，可以使用以下命令加入它：
