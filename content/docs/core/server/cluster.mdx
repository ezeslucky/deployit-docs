...
When you deploy applications in deployit, all of them run on the same node. If you wish to run an application on a different server, you can use the cluster feature.

The idea of using clusters is to allow each server to host a different application and, using Traefik along with the load balancer, redirect the traffic from the deployit server to the servers you choose.

## Server Scaling Methods

There are two primary ways to scale your server:

1. **Vertical Scaling**: This involves adding more resources to the same deployit server, such as more CPU and RAM.
...
To perform vertical scaling, you need to add more resources to your deployit server, that is, more CPU and RAM. This is done through your VPS provider.
...
1. deployit server running (Manager).
2. Have at least one extra server with the same architecture as the deployit server.
...
1. A user.
2. A password.
3. A domain. Ensure this domain is pointing to the deployit VPS.
...
You can click the 'Add Node' button, which will display the instructions you need to follow to add your servers as nodes and join them to the deployit manager node.
